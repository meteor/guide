...
If you want to distribute and reuse code that you've written for a Meteor application, then you should consider publishing that code on npm if it's general enough to be consumed by a wider JavaScript audience. It's simple to [use npm packages in Meteor applications](using-npm-packages.html), and possible to [use npm packages within Atmosphere packages](writing-atmosphere-packages.html), so even if your main audience is Meteor developers, npm might be the best choice.
