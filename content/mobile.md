...
The port mentioned above will be generated based on your app id that is hash string inside `.meteor/.id` file in your project. If you need to run multiple apps in the same device using the same source code you need to specify a port for each different app, that is possible using `--cordova-server-port` option in the Cordova commands `run` and `build`, otherwise your code will not be able to run multiple apps in the same time on iOS (if you see this error on XCode console `Failed binding IPv4 listening socket: Address already in use (48)` you are probably facing this issue).
